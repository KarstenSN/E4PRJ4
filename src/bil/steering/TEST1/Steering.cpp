...
// 	max_speed_ = (int)settingsPtr_->getMaxSpeed();
	...
	else
	{
		if((float)((max_speed_/255)*UsrInput_->forward) != speedReqFor_ || (float)((max_speed_/255)*UsrInput_->reverse) != speedReqBack_)
		{
		      ...
		      speedReqFor_ = (int)((float)(max_speed_/256) * (float)UsrInput_->forward); 	//(maxSpeed/255)*forward adjust input with max speed 
		      speedReqBack_ = (int)((float)(max_speed_/256) * (float)UsrInput_->reverse);	//(maxSpeed/255)*reverse adjust input with max speed
...
{	
 ...
 // while(testwhile==){}
	logPtr_->writeEvent(__PRETTY_FUNCTION__, "PWMUpdate entry");
